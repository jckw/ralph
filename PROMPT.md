...
6. Run quality checks (e.g., typecheck, lint, test - use whatever your project
   requires)
    Run tests selectively - the test suite is very large and you may be better
    off using --changed or running specific tests. For large integration work
    you may want to run more tests.
...
- You may use **up to 50 parallel subagents** for all operations.
- You may use **only 1 subagent for tests**.
- Do **not** bother building the project, but you **may run type checks** (also limited to a single agent).
